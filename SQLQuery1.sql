...
/* 1. amount spend by each person on zomato*/
...
/* 4. most purchased item and no. of times it is purchased by any customer*/
select userid , count(product_id) cnt from sales where product_id =
(select top 1  product_id  from sales group by product_id order by count(product_id) desc)
group by userid;

/* 5. item which is popular for each customer*/
...
/* 6. which  item was purchased first when they became a member*/
select * from 
...
/* 8. what is the total orders and amount spent by each person before becoming a member*/

select userid, count(created_date) order_purchased, sum(price) total_amt_spent from
(select c.* ,d.price from
(select a.userid , a.created_date, a.product_id , b.gold_signup_date from sales a inner join goldusers_signup b on 
a.userid = b.userid and created_date<=gold_signup_date) c inner join product d on c.product_id = d.product_id)e
group by userid

/* 9. if buying each product generates points for eg 5rs=2 zomato points and each product has different purchasing points
for eg p1 5rs=1 zomato points , for p2 10rs = 5zomato point and p3 5rs=1 zomato point

Calculate points collectd by each customers and for which product most points have been given till now */
...
(select c.userid , c.product_id , sum(price) amt from
(select a.* ,b.price from sales a inner join product b on a.product_id=b.product_id) c
group by product_id, userid) d) e)f group by product_id) f)g where rnk=1 ;

/* 10. in the first one year after a customer joins the gold program (include their joining date)
irrespective of what the customer has purchased, they earn 5 zomato points for every 10rs. spent .
Who earned more than 1 or 3 and what was their points earning in their first year? 
1 zp=2rs
0.5zp =1rs
*/
...
/*11. rank all the transaction of all the customers*/

select * , rank() over( partition by userid order by created_date) rnk from sales

/*12. rank all the transactions for each member whenever they are a zomato member for every non gold member 
transaction marked as na */
